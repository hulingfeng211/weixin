...
class MessageHandler(WeiXinRequstHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        response=yield AsyncHTTPClient().fetch(send_message_url,method="POST",body=body,**self.settings[constant.PROXY_SETTINGS])
        if response:
            self.write(response.body)
class MenuHandler(WeiXinRequstHandler):
    @coroutine
    def get(self, *args, **kwargs):
        create_menu_url=constant.CREATE_MENU_URL%(self.access_token,self.settings[constant.WEIXIN_SETTINGS][constant.AgentId])
        body=json.dumps(get_menu(),ensure_ascii=False).encode('utf-8')
        response=yield AsyncHTTPClient().fetch(create_menu_url,method="POST",body=body,**self.settings[constant.PROXY_SETTINGS])
        if response:
            self.write(response.body)
...
class MediaHanlder(WeiXinRequstHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        response=yield  AsyncHTTPClient().fetch(url,method="POST",
                                                body=json.dumps(body,ensure_ascii=False).encode('utf-8'),**self.settings[constant.PROXY_SETTINGS])
        if response:
            self.write(response.body)
